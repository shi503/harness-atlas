...
published dependency runs upward ([`3b` Routing](./content/component-07-routing.md) resolves against
...
|---|---|---|---|
| **0 Foundation** | [`0a` Substrate](./content/component-01-substrate.md) | What do we run on? | `shipped` |
| **1 Environment** | [`1a` Environment](./content/component-02-environment.md) | What can we reach? | `bet` |
| **2 Agent Harness** | [`2a` Adapters & Middleware](./content/component-03-adapters-and-middleware.md) | How does the loop reach any of it? | `shipped` |
| | [`2b` Hooks](./content/component-04-hooks.md) | When does something happen without anyone remembering to run it? | `shipped` |
| | [`2c` Enforcement](./content/component-05-enforcement.md) | Where does a rule stop being a request? | `shipped` |
| **3 System Stacks** | [`3a` Control](./content/component-06-control.md) | How does intent become work that is allowed to start? | `shipped` |
| | [`3b` Routing](./content/component-07-routing.md) | Who picks this up? | `emerging` |
| | [`3c` Composition](./content/component-08-composition.md) | What is this agent made of? | `emerging` |
| | [`3d` Configuration](./content/component-09-configuration.md) | Where does our opinion attach? | `shipped` |
| | [`3e` Standards](./content/component-10-standards.md) | What does good look like here, before anyone writes anything? | `bet` |
| **4 Capabilities** | [`4a` Capability](./content/component-11-capability.md) | What can the team do, and how does it travel? | `shipped` |
| | [`4b` Capability Permissions](./content/component-12-capability-permissions.md) | Who may invoke this, and against what? | `emerging` |
| **5 Context ⟳** | [`5a` Individual Memory](./content/component-13-individual-memory.md) | What do I know that the team has not agreed to? | `emerging` |
| | [`5b` Team Memory](./content/component-14-team-memory.md) | What has to be true for anyone else's agent? | `emerging` |
| | [`5c` Knowledge](./content/component-15-knowledge.md) | Where does an agent go to find out what we know? | `shipped` |
| **6 Workspaces ⟳** | [`6a` Product](./content/component-16-product.md) | Where does the work land, and what is it not allowed to become? | `bet` |
| | [`6b` Infrastructure](./content/component-17-infrastructure.md) | Where does the work actually happen? | `emerging` |
| | [`6c` Estate](./content/component-18-estate.md) | What code exists, and what does a change here break there? | `emerging` |
| | [`6d` Delivery](./content/component-19-delivery.md) | How does a finished change get to production? | `bet` |
| **7 Workflow Tasks** | [`7a` Workflow Tasks](./content/component-20-workflow-tasks.md) | What is the unit of work, written down? | `emerging` |
| **8 Trust** | [`8a` Evals](./content/component-21-evals.md) | What does the work have to clear before it ships? | `shipped` |
| | [`8b` Evidence](./content/component-22-evidence.md) | How does someone who was not there come to believe it? | `shipped` |
| | [`8c` Observability](./content/component-23-observability.md) | What did it actually do? | `shipped` |
| | [`8d` Efficiency](./content/component-24-efficiency.md) | What did that cost, and was it worth buying again? | `emerging` |
| **9 IMPROVE** | [`9a` Learning](./content/component-25-learning.md) | What happens to a lesson after it is learned? | `emerging` |
| | [`9b` Rituals](./content/component-26-rituals.md) | Who is in the loop, and what are they there to do? | `emerging` |
| | [`9c` Cadence](./content/component-27-cadence.md) | What runs without anyone asking? | `shipped` |
| | [`9d` Anti-fragile Lifecycle](./content/component-28-anti-fragile-lifecycle.md) | What closes the loop? | `bet` |
| | [`9e` Raise the Floor](./content/component-29-raise-the-floor.md) | How does a second way of doing something get retired? | `bet` |
| | [`9f` Diagnose the Bottleneck](./content/component-30-diagnose-the-bottleneck.md) | What is limiting us, and what do we fix next? | `bet` |
| **10 Teams & Agents** | [`10a` Roster](./content/component-31-roster.md) | Who is on this team? | `emerging` |
| | [`10b` Org](./content/component-32-org.md) | Who answers for this, and who may change it? | `claimed` |
| **11 Surfaces** | [`11a` Surfaces](./content/component-33-surfaces.md) | Where is the work seen, and which version of it is true? | `emerging` |
...
outcomes land; [`6a`](./content/component-16-product.md) is that layer.
...
candidate is [`8b` Evidence](./content/component-22-evidence.md) — its ledger grows monotonically with
...
and dated 2026-09-01; argued in full at [`5a`](./content/component-13-individual-memory.md).
...
[`1a`](./content/component-02-environment.md) and the reach is derived at
[`2a`](./content/component-03-adapters-and-middleware.md), graded apart; the argument is owned at `1a`.

**D · Actors and Org are separate** *(layer 10)*. *Who exists* ([`10a`](./content/component-31-roster.md))
and *who answers* ([`10b`](./content/component-32-org.md)) are two rows that fail apart, and each file
...
that checks it ([`3e`](./content/component-10-standards.md)).
...
|---|---|---|
| **Context Files** | [`5b` Team Memory](./content/component-14-team-memory.md) | *"Markdown file loaded into the context each session"* — committed to the repo, owned by the team, read by every agent. The per-user store is `5a`; the surveyed artifact is team-addressed |
| **Settings** | [`2c` Enforcement](./content/component-05-enforcement.md) | *"JSON/TOML config for project-level tool behavior"* — the file where the harness's posture binds: permission and deny rules, managed overrides, sandbox flags. The cheap rungs of `2c`'s ladder live here; what else it carries configures machinery, which is not graded |
| **Skills** | [`4a` Capability](./content/component-11-capability.md) | *"Reusable knowledge and invocable workflows"* — the catalog's unit. The uncomfortable measurement that most published skills ship no executable resource is owned at `4a` |
| **Subagents** | [`3c` Composition](./content/component-08-composition.md) | The survey's sharpest sentence is an isolation claim — subagents *"operate in parallel to the central agent loop, in their own context"* — and deciding what shares a context window is `3c`'s definition. Not `10a`: a list of agent definitions is not a roster, as `10a`'s own file rules |
| **Commands** | [`7a` Workflow Tasks](./content/component-20-workflow-tasks.md) | *"User-triggered shortcuts for predefined prompts"* — a person triggers a named piece of work, and the prompt behind it is the work contract in its weakest form. The strongest published instances are workflow entries: handoffs, phase prompts |
| **Hooks** | [`2b` Hooks](./content/component-04-hooks.md) | Pre-ruled. *"Scripts executed at specific agent lifecycle points"* — the harness defines the points, so the mechanism is the harness layer's |
| **Rules** | [`3e` Standards](./content/component-10-standards.md) | *"System-level instructions to control agent behavior"* — instructions, not mechanisms: the pre-hoc Guides half of the `O-5` split, and the surface a path-scoped standard lives on |
| **MCP** | [`1a`](./content/component-02-environment.md) declared · [`2a`](./content/component-03-adapters-and-middleware.md) adapted | Pre-ruled, and the split is the argument: an entry with an owner and an auth model at `1a`, a per-call resolution bound to the loop at `2a`. A tool list discovered at call time is reach without inventory |
...
[`11a` Surfaces](./content/component-33-surfaces.md) is the only exception. That is one fact, not
...
[`3a` Control](./content/component-06-control.md); **the work contract they read is the primitive**, and
it is graded at [`7a`](./content/component-20-workflow-tasks.md). The same line splits
[`9c` Cadence](./content/component-27-cadence.md): the trigger is machinery, and the row is graded on
...
harnesses; [`6c` Estate](./content/component-18-estate.md) and
[`6d` Delivery](./content/component-19-delivery.md) are code-native by definition.
...
[`3e`](./content/component-10-standards.md)'s inheritance contract and
...
on [`5a` Individual Memory](./content/component-13-individual-memory.md): run a different harness,
different adapters, your own control stack, your own notes. Three things conform, under **tighten,
never loosen**: [`3e` Standards](./content/component-10-standards.md) (*reference, never copy; tighten,
never contradict*), [`5b`](./content/component-14-team-memory.md)'s governed tier including the decision
ledger (when copies disagree, the ledger wins), and [`6a` Product](./content/component-16-product.md)'s
