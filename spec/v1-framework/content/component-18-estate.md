...
> that — **it makes the correlation visible instead of arguable** ([`CROSSWALK.md`](../CROSSWALK.md)
> §3.6). The distinction that survives a one-repo team is the one below.

**The test, stated so a one-repo team can run it.** [`6a`](./component-16-product.md) Product answers
...
piece of work belongs to*; [`3b`](./component-07-routing.md) resolves *which actor picks it up*. Both
...
decomposed ([`CROSSWALK.md`](../CROSSWALK.md) §2).

**What this layer is not.** It is not [`1a`](./component-02-environment.md) Environment. **Estate is
...
| **ours** | `registry/repositories.yaml` behind a schema, with registry-driven clone — **genuinely built, and two repos wide.** Ownership is recorded; impact analysis is not, so the differentiated third of the function is the third we do not have | [`systems/loomwarp.md`](https://github.com/shi503/loomwarp-team-system/blob/master/projects/loomwarp/references/comparisons/systems/loomwarp.md) §*Architecture* · [`02-component-matrix.md`](../../../comparisons/02-component-matrix.md) §1 |
