...
[`2c`](./component-05-enforcement.md) Enforcement, in the harness, and it presents twice: **Guides**
pre-hoc at [`3e`](./component-10-standards.md), **Sensors and Evals** post-hoc here. That is Böckeler's
...
> enforcement is not one of its rows are settled at [`2c`](./component-05-enforcement.md)
> ([`CROSSWALK.md`](../CROSSWALK.md) §2.1).
...
split and its two failure modes are argued at [`2c`](./component-05-enforcement.md); what this row owns
...
that holds, `8a` raises the floor and stops**, which is exactly why [`8b`](./component-22-evidence.md)
exists as a separate row and why the ordering of human review is [`9b`](./component-26-rituals.md)'s.

**What this layer is not.** It is not `3e`, which authors the standard. It is not
[`8b`](./component-22-evidence.md), which records what happened — a verdict is a judgement, a record is
...
| **ours** | `standards/definition-of-done.md` and `standards/evaluation-doctrine.md` — **the bar written, the sensor absent.** The doctrine describes an eval pyramid that a vendor has since implemented and we have not; `events.jsonl` classifies outcomes by regexing markdown, which is a reading rather than a gate | [`systems/loomwarp.md`](https://github.com/shi503/loomwarp-team-system/blob/master/projects/loomwarp/references/comparisons/systems/loomwarp.md) §*Architecture* |
