...
- the comparison table: for each harness in `harnesses/`, its **named primitive** for this
  component, the citation, and the verbatim line that proves it — every cell linking into a Tier-3
  anchor;
...
`5`→`6`, Workspaces `6`→`7`; all three permute together, one atomic pass). If W0 struck this, keep
the old twelve. Either way the skill's inline list and the grid must match.
...
- **AC-2** Every table cell that claims a primitive links to an anchor in `harnesses/` that exists.
