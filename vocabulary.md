...
for · **UNDERCOUNT** the vendor names it, our profile's primitive table (§C in Template A, §5 in v2) did not · **DRAFT** the only source is a draft.
...
|---|---|---|---|---|
| `workspace` | "Directory containing `SOUL.md`, `AGENTS.md`, `USER.md`, and local files" — a per-agent config directory | OpenClaw | `content/openclaw.md` §5 *Agent* | `3d` Configuration |
| `workspace` | "Files persist in a shared workspace at `/workspace`… All of your Bots use the same persistent cloud computer" — a shared VM filesystem root | Grok Bot | `content/grok.md` §5 | `6b` Infrastructure |
...
| `workspace` | `experimental_workspace` — a plugin API for pluggable isolated execution environments | OpenCode | `content/opencode.md` §6 `6b` | `6b` Infrastructure |
...
| `session` | "Sessions are stored as JSONL files with a tree structure… `id` and `parentId`, enabling in-place branching" — the transcript, which is also the run receipt | Pi | `content/pi.md` §5 | `5a` Individual Memory · `8b` Evidence |
...
| `session` | "Session key is a routing selector, not an authorization token" — a routing key for a message stream | OpenClaw | `content/openclaw.md` §5 | `3b` Routing |
| `session` | "When an agent is *running* it is a **session** — a live process the platform can start, stop, prompt, and observe" | Gas City | `content/gas-city.md` §5 *(supporting)* | `6b` Infrastructure |
| `session` | The loop's own unit — `Codex` / `Session` / `Task` / `Turn`; the persisted record is the **Thread** | Codex | `content/codex.md` §7, §C | `2a` Adapters & Middleware (loop) · `8b` (Thread) |
| `agent` | A markdown file with frontmatter (`mode: primary \| subagent \| all`) — agent-as-file | OpenCode, Claude Code, Grok | `content/opencode.md` §5 · `content/grok.md` §5 | `3c` Composition · `10a` Roster |
| `agent` | `agents.entries.<agentId>` with a `workspace` directory and an `agentDir` — agent-as-directory | OpenClaw | `content/openclaw.md` §5 | `3c` · `10a` |
| `agent` | "WHO — a configured worker — name, provider, prompt template, scope — pure configuration" — agent-as-tuple | Gas City | `content/gas-city.md` §5 | `3c` · `10a` |
| `command` | A slash-command template: frontmatter `template`, `$ARGUMENTS`, invoked `/name` | OpenCode | `content/opencode.md` §5 | `4a` Capability |
...
| `bot` | "Bot Mode" — a roster of named Bots, each a profile | Hermes | `content/hermes.md` §6 `10a` **UNDERCOUNT** | `10a` Roster |

### 1.2 Synonym sets — one row per concept

| Concept | The words, by system | Where | Component |
|---|---|---|---|
| **Instruction files** the loop reads at start | Pi `context file` (`AGENTS.md`/`CLAUDE.md`, concatenated up the tree) · Hermes `context file` · Grok Build `project rules` · OpenCode `rules` · OpenClaw `workspace bootstrap files` · Codex `AGENTS.md` (+ `AGENTS.override.md`) · Claude Code `CLAUDE.md` + `.claude/rules/` · generic-cerebro `rule file` (glob-scoped, kept distinct) | `content/{pi,hermes,grok,opencode,openclaw,codex}.md` §5 · `content/claude-code/07-context-and-memory.md` · ↪ `comparisons/systems/kd-built-frameworks/02-generic-cerebro.md` | `3d` Configuration |
| **Skill** — the Agent Skills standard | Pi · OpenCode · Grok · Claude Code · Hermes · OpenClaw · Codex all say *skill*; Gas City symlinks skills into each provider's convention | every profile §5; `content/gas-city.md` §6 `4a` | `4a` Capability |
| **Hooks** — named separately or folded | *Named*: Claude Code, Hermes, OpenClaw, Grok (fail-open), Codex (11 events) · *Folded into one extension object*: Pi `extension`, OpenCode `plugin`. A design disagreement, not a labelling one | `content/{hermes,openclaw,grok,codex}.md` §5 · `content/pi.md` §5 *Extension* · `content/opencode.md` §5 *Plugin* | `2b` Hooks |
| **One container, many resource types** | Pi `pi package` · Claude Code `plugin` · Grok `plugin / marketplace` · OpenClaw `plugin` (ClawHub) · Hermes `plugin` · Codex `plugin` (bundles skills, MCP server) · Gas City `pack` ("the City *is* a pack") | profiles §5 | `4a` Capability |
| **Permission posture** — three tiers under different names | Claude Code `permission mode` (default · acceptEdits · plan · auto · dontAsk · bypassPermissions) · OpenCode `permission` (allow · ask · deny; deny survives `--auto`) · Grok Build `permission rule / mode` · Grok Bot `approval / auto-review rule` · Codex `permission profile` (+ `sandbox_mode`, coexisting) · Hermes `toolset` · OpenClaw `tool policy / exec approvals` · QM `posture` (strict · auto · dangerous, monotonic narrowing) · Indigo HQ `hook profile` (minimal · standard · strict) | profiles §5 · ↪ `comparisons/systems/{qm,indigo-hq}.md` | `2c` Enforcement · `4b` Capability Permissions |
| **Delegation object** | Claude Code `subagent` (+ `agent team`, experimental) · Codex `subagent` · Grok `agent / subagent / persona / role` (three objects where the row wants one) · Hermes `profile` · Pi: example only, not shipped (`EX/subagent`) | profiles §5 | `3c` Composition |
| **Cadence** | Hermes `cron job` · OpenClaw `automation (cron) / heartbeat` · Grok Bot `routine` · Gas City `order` (supporting) | profiles §5 | `9c` Cadence |
...
| **Memory files** | Hermes `MEMORY.md` / `USER.md` (hard caps, frozen snapshot at start) · OpenClaw the same filenames ("by lineage") · Claude Code auto memory · Codex `memories/` (git-baselined, two-phase, no approval gate) | `content/{hermes,openclaw,codex}.md` §5 · `content/claude-code/07-context-and-memory.md` | `5a` Individual Memory |
| **Identity file** | `SOUL.md` — Hermes ("slot #1 in the system prompt") and OpenClaw, same word, same slot | `content/{hermes,openclaw}.md` §5 | `3d` Configuration |
...
| **Trust gate on project-local config** | Pi `project trust` ("It is not a sandbox") · Claude Code `workspace trust` · OpenCode: project `.opencode` loads by default | `content/pi.md` §5 · `content/claude-code/08-policy-and-governance.md` | `2c` Enforcement |

### 1.3 Orphans — the five nouns no grid has a row for

| Term | Concept (vendor's words) | Who | Where | Ruling |
|---|---|---|---|---|
| `gateway` | "A single long-lived Gateway owns all messaging surfaces… channel connections, config, credentials, and the control-plane API" | OpenClaw (primitive) · Hermes (`gateway.profile_routes`, never in its §C — **UNDERCOUNT**) | `content/openclaw.md` §5 · `content/hermes.md` §6 `3b` | **An altitude**, not a component (`04-harness-alignment.md` §4.1: *gateway / host*). Inside a profile the gateway's jobs land at `3b` Routing · `11a` Surfaces · `10b` Org |
| `runtime` | "owns one prepared model loop: it receives the prompt, drives model output, handles native tool calls, and returns the finished turn" | OpenClaw (primitive) · Grok Build (`xai-grok-shell`, a crate description) | `content/openclaw.md` §5 · `content/grok.md` §7 | **An altitude** (§4.1: *runtime*). The loop itself is `2a`; the model under it `0a` |
| `sandbox` | OS-level isolation — "Landlock on Linux, Seatbelt on macOS… The kernel enforces these limits for the process lifetime" (Grok); "not a perfect security boundary, but it materially limits filesystem and process access" (OpenClaw); "a separate layer from permission rules" (Claude Code); Codex Seatbelt · Landlock/bwrap · Windows token; QM "durable sandbox" per scope. **Refused** by Pi: "Run in a container, or build your own confirmation flow" | Grok Build · OpenClaw · Claude Code · Codex · QM ↪ · Pi (refusal) | `content/{grok,openclaw,codex,pi}.md` §5 · `content/claude-code-draft.md` **DRAFT** | **A synonym set, absorbed**: `2c` Enforcement when kernel-enforced; `6b` Infrastructure for the container path |
...
|---|---|---|---|---|
| `bead` | "WHAT — one unit of work — ID, title, status, type — the universal substrate: tasks, mail, sessions, convoys are all beads differing only by `type`"; hash IDs, hierarchical, atomic claiming, typed relations | Gas City / Beads | `content/gas-city.md` §5 | `7a` Workflow Tasks · `3a` Control (convoy, board) · `5a` via `bd remember` / `bd prime` |
...
|---|---|---|---|---|
| `gateway` | `gateway.profile_routes`, `gateway-config.yaml`, "the gateway never opens an inbound port" | Hermes | `content/hermes.md` §6 `3b` `3d` `6b` | `3b` Routing |
| `/goal` | a goal with a token budget and an independent evidence review | Hermes | `content/hermes.md` §6 `3a` | `3a` Control |
| "Bot Mode" | a roster of named Bots | Hermes | `content/hermes.md` §6 `10a` | `10a` Roster |
| audit ledger | "never stores prompts, message bodies, tool arguments, tool results" — metadata only | OpenClaw | `content/openclaw.md` §6 `8b` | `8b` Evidence |
| operator roles | creator · owner · participant; session *owner* "in the style of a GitHub issue assignee" | OpenClaw | `content/openclaw.md` §6 `10b` | `10b` Org |
| `rig` | "WHERE — an external project (usually a git repo) registered with the city" | Gas City | fixed in `content/gas-city.md` §5 on 2026-09-03 | `6c` Estate |
...
| `constraint form` | *Whether intent is imposed by inference or by machine* | ours (DX-2) | `spectrums/01-scorecard.md` §4 | **RETIRED** 2026-09-07, claimed the same day. The question survives as axis III `binding force`, which already owned it; the DX-2 slot is now `weight class` | not a component |
| `weight class` | *How much harness this is, and what production quality it is built to produce* — bulk, complexity, inference cost, expected rigor | ours (DX-2) | `spectrums/01-scorecard.md` §4 | **CLAIMED** 2026-09-07 — zero prior occurrences. Named on the fighter-weight-class analogy; deliberately not `posture` (three referents already) or `complexity` (grades) | not a component |
| `surfaces & extendability` | *How many interaction points, extension points and execution environments a harness supports* — surfaces, hooks, plugins, protocols, environments | ours (DX-3) | `spectrums/01-scorecard.md` §4 | **CLAIMED** 2026-09-07, superseding `footprint`. KD's spelling *extendability* is kept over *extensibility* by ruling, so it is not silently normalised | not a component |
| `org scale` | *Whether a harness is built for one operator or for many users across an organization* | ours (DX-1) | `spectrums/01-scorecard.md` §4 | **CLAIMED** 2026-09-07, superseding `operator scale` **as a DX id only** — axis I still carries `operator-scale`, and ending that collision is what the retirement was for | not a component |
| `domain specialization` | *Whether a harness is general-purpose or specialized to one named domain*, read from positioning **and** from domain-shaped machinery | ours (DX-4) | `spectrums/01-scorecard.md` §4 | **CLAIMED** 2026-09-07, superseding `domain breadth` — **and running the opposite way**: `+3` is now specific, where `domain breadth`'s `+3` was general | not a component; the categorical reading is `04-harness-alignment.md` §1's `Genre` row |
| `cost controls & efficiency` | *Whether a harness ships cost observability, token-efficiency gains, and model measurement and routing* | ours (DX-7) | `spectrums/01-scorecard.md` §4 | **CLAIMED** 2026-09-07, superseding `cost visibility` **as a DX id only** — axis X keeps `cost-visibility`, and this dimension is wider than it: mechanisms, never price | not a component; closest grid row is `8d` Efficiency |
...
**Pi** — `content/pi.md` §5 · 8 + 3 (re-run pending, ISSUE-007)
...
**Hermes** — `content/hermes.md` §5 · 10 + undercounts
...
**OpenClaw** — `content/openclaw.md` §5 · 12
...
**OpenCode** — `content/opencode.md` §5
...
**Grok Build / Grok Bot** — `content/grok.md` §5 / §C.2 · 8 / 6
...
**Codex** — `content/codex.md` §5 · 8 + 4 (⚠️ contestable)
...
**Gas City** — `content/gas-city.md` §5 · 6 + 5 (healthy; own admission test; one documented deletion)
...
**FRACTAL** — `content/fractal.md` §5 · 5 + 3, never stated as a set (⚠️ contestable)
...
**LoomWarp** — `content/loomwarp.md` §5 · **0 named** — stated once in a superseded spec, then dropped
