...
> **Reorganised 2026-09-08.** The four repo-wide rulings moved from the repository root into this
> folder, and **each filename is now its ruling id** — `2026-09-08-archival.md` is ruling
> `2026-09-08-archival`. Citations by id and by path are the same string, which is what the old
> `RULING-` prefix cost. This file was `RULINGS.md` at the root. Nothing about a ruling's authority
> changed; only where its text sits.

**Why an index and not a migration.** All three homes are legitimate: `rulings/<date>-<slug>.md`
for a repo-wide decision; a `### N ✅ RULED <date> — …` heading inside
...
the formats — it was **findability**: the issue log cites *"KD's ruling,
...
archaeology. **A ruling that is not indexed here is a defect.**

**Every ruling's text must live in a tracked file.** `.claude/` is gitignored, so a ruling decided in
a workstream session is written out to `rulings/<id>.md` rather than cited where it was said. Five were
re-homed that way on 2026-09-08 when the process record went local.
...
| 2026-09-02 | `2026-09-02-spinout` | The harness framework spins out of LoomWarp into this repo; a maturity spectrum and a primitive catalog are un-fused | [`rulings/2026-09-02-spinout.md`](./2026-09-02-spinout.md) | PERMANENT |
...
| 2026-09-04 | `2026-09-04-w3-q3` | An UNDERCOUNT is flagged in the vocabulary ledger; the profile is corrected when it is next re-read, not retro-fitted | [`2026-09-04-w3-q3.md`](2026-09-04-w3-q3.md) | PERMANENT |
| 2026-09-04 | `2026-09-04-w3-q4` | An unnamed `●` in a grid becomes `◐` with a dated note — a correction, not a re-score. `0a` keeps `●` under the stated convention | [`2026-09-04-w3-q4.md`](2026-09-04-w3-q4.md) | PERMANENT |
| 2026-09-04 | `2026-09-04-template-v2` | Profiles take Template v2: card → system map → workflows → `●◐○` matrix → primitives → collapsed details. Coverage marks and source marks never share a table | [`skills/harness-teardown/SKILL.md`](../skills/harness-teardown/SKILL.md) | PERMANENT |
| 2026-09-04 | `2026-09-04-layout-b` | A `<details>` may not contain a heading — GitHub scrolls to an anchor inside a collapsed block without opening it. Headings stay outside; only the body collapses | [`2026-09-04-layout-b.md`](2026-09-04-layout-b.md) | PERMANENT |
...
| 2026-09-07 | `2026-09-07-alignment-reference` | `components/ALIGNMENT.md` §1 is superseded as a rubric by the DX scorecard, and **kept as a reference** — the prose inventory and findings-generator. Not re-headed, not archived | [`components/ALIGNMENT.md` §1](../components/ALIGNMENT.md#1-who-they-are-and-whether-they-are-peers) | PERMANENT |
...
---
| 2026-09-08 | `2026-09-08-archival` | The v1 specification, `craft/`, the 22 ADR drafts and the two pre-template profile drafts are **archived**. And the rule the precedent could not settle: **a whole-directory archival re-heads by index and frontmatter, and banners only the files carrying inbound citations from live material** — the banner exists to catch a reader arriving from a citation, and 42 banners nobody reads is how a warning stops being read. Two live registers left the specification first, headings verbatim, because a rule the live tier reads cannot sit under `archive/` | [`rulings/2026-09-08-archival.md`](./2026-09-08-archival.md#re-heading--the-rule-this-ruling-settles) | PERMANENT |
| 2026-09-08 | `2026-09-08-id-register` | **The component ID register is `components/00-README.md`**, not `CROSSWALK.md` §0, which `CLAUDE.md` had named for six days. §0 is a *derivation* — an argument for the ID space — and the corpus was already running on two other sources: each page's `sublayer:` frontmatter and the teardown skill's inline checklist, which states it *"is never read from a spec."* Those two were written eight days apart from different material and **agree on all 33 ids and all 33 names**; the register is generated by reconciling them, and the skill wins if they ever diverge | [`components/00-README.md` §The roster](../components/00-README.md#the-roster--this-file-is-the-register) | PERMANENT |
| 2026-09-08 | `2026-09-08-deep-read-claims` | The deep read's **highlighted gate is retired** for a per-commission depth negotiation; the set carries the vendor's claim ledger, and its guide maps claim → mechanism. **That mapping records, it does not grade** | [`rulings/2026-09-08-deep-read-claims.md`](./2026-09-08-deep-read-claims.md#3-the-claim-test-records-it-does-not-grade) | PERMANENT |
| 2026-09-08 | `2026-09-08-authorship-provenance` | **R11's `verification:` block extends from the scorecard to every authored analysis** — profiles, deep-read documents, positions. The backfill of files whose model was never captured is an **attestation** and says so in `note:`; going forward `drafted_by` is captured at write time | [`rulings/2026-09-08-authorship-provenance.md`](./2026-09-08-authorship-provenance.md#3-the-backfill-is-an-attestation-and-the-files-say-so) | PERMANENT |
| 2026-09-08 | `2026-09-08-mit-educational` | **One MIT licence for the whole repository**, plus an **educational-purposes** statement — a research and teaching instrument, not a buyer's guide and not an endorsement. The two-licence split recommended by W11 (CC-BY-4.0 prose + MIT code) is **superseded**; its surviving point — a licence does not relicense quoted vendor material — moves to `NOTICE`. *"Educational purposes" is a statement of purpose, not a restriction: MIT permits commercial use, and writing a restriction into the grant would make it something other than MIT* | [`NOTICE`](../NOTICE) · [`LICENSE`](../LICENSE) | PERMANENT |
