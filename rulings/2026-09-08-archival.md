...
**Why now rather than at the spin-out.** `rulings/2026-09-02-spinout.md` already anticipated it — of
...
|---|---|---|
| `CROSSWALK.md` §3 | [`components/CROSSWALK.md`](../components/CROSSWALK.md) | Six ruling **texts** — `rulings/00-README.md` §*Why an index* names this file as one of three sanctioned homes for one — plus §3.13, the register where a 34th component waits |
...
Carrying the headings verbatim meant **all seven anchors `rulings/00-README.md` points into §3 still resolve**;
only the path changed. That is why this was preferable to relocating six ruling texts into root
`RULING-*.md` files, which `rulings/00-README.md` argues against by name as *"large, low-value churn"* and which
